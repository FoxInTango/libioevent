...
# ** 项目配置区 **
#    输出文件名称
TARGET_NAME     = libioevent
#    输出文件类型
TARGET_TYPE     = ${TARGET_TYPE_DLL}
#    输出文件后缀 [自动判别]
TARGET_NAME_EXT =
#    安装位置
INSTALL_PATH    = /usr/local/libioevent
...
# 链接库配置
TARGET_LIB_INCLUDE = # 示例: -I/usr/local/libioevent/include
TARGET_LIB_BINARY  = # 示例: -L/usr/local/libioevent/lib
TARGET_LD_FLAGS    = # 示例: -L/usr/local/libioevent/lib

TARGET_LIB_INCLUDE = # 示例: -I/usr/local/libcipher/include
...
${TARGET_LIBRARY_DIR}/${TARGET_NAME} : $(TARGET_OBJECTS) $(THIRDS_OBJECTS)
	cc ${TARGET_LD_FLAGS} ${TARGET_LIB_PIC} ${TARGET_LIB_FLAG} -o $@ $(TARGET_OBJECTS) ${THIRDS_OBJECTS} ${TARGET_LIBS}
 
${TARGET_OBJECTS_DIR}/%.o : $(TARGET_SOURCES_DIR)/%.cpp
	cc ${CFLAGS} ${CXXFLAGS} -c $< -o $@ ${TARGET_LIB_PIC}

IOServer : ./out/main.o
	cc -o bin/qr ./out/main.o ${TARGET_LIBS}
